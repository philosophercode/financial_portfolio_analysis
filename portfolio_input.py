...
class PortfolioInputHandler:
    ...
    def load_portfolio_from_file(self, file_path: str) -> Tuple[List[str], Dict[str, float], float]:
        """
        ...
        try:
            ...
            if self._is_shares_format(df):
                ...
                raise ValueError("Unknown CSV format. Expected columns: 'symbol,shares' or 'symbol,weight'")
                
        except Exception as e:
            raise RuntimeError(f"Error loading portfolio from {file_path}: {e}")
    
    def create_portfolio_from_dict(self, 
                                 portfolio_dict: Dict[str, Union[int, float]], 
                                 input_type: str = "auto",
                                 total_value: float = None) -> Tuple[List[str], Dict[str, float], float]:
        """
    ...
    def _is_shares_format(self, df: pd.DataFrame) -> bool:
        ...
        return all(col.lower() in [c.lower() for c in df.columns] for col in required_cols)
    ...
    def _process_shares_format(self, df: pd.DataFrame) -> Tuple[List[str], Dict[str, float], float]:
    ...
    def _process_weights_format(self, df: pd.DataFrame) -> Tuple[List[str], Dict[str, float], float]:
    ...
    def _calculate_total_value_from_shares(self, shares_dict: Dict[str, int]) -> float:
        ...
        for symbol, shares in shares_dict.items():
            if symbol in self.current_prices:
                ...
                print(f"Warning: Could not get price for {symbol}, skipping from total calculation")
        
        return total_value
    
    def _shares_to_weights(self, shares_dict: Dict[str, int], total_value: float) -> Dict[str, float]:
...
def load_and_analyze_portfolio(file_path: str, 
...
def compare_portfolios(current_file: str, 
                      optimization_method: str = "max_sharpe",
    ...
    handler = PortfolioInputHandler()
    symbols, current_weights, total_value = handler.load_portfolio_from_file(current_file)
    
    # Get price data
    fetcher = DataFetcher()
    price_data = fetcher.get_multiple_stocks(symbols, source="yahoo", period=config.DEFAULT_PERIOD)
    ...
    print(f"{'Expected Return':<25} {current_ret:<14.2%} {opt_ret:<14.2%} {opt_ret-current_ret:<14.2%}")
    print(f"{'Volatility':<25} {current_vol:<14.2%} {opt_vol:<14.2%} {opt_vol-current_vol:<14.2%}")
    print(f"{'Sharpe Ratio':<25} {current_sharpe:<14.3f} {opt_sharpe:<14.3f} {opt_sharpe-current_sharpe:<14.3f}")
    ...
    if show_plots:
        ...
        if current_clean:
            ax1.pie(current_clean.values(), labels=current_clean.keys(), autopct='%1.1f%%')
        ...
        x = range(len(metrics))
        width = 0.35
        ax3.bar([i - width/2 for i in x], current_vals, width, label='Current', alpha=0.7)
        ax3.bar([i + width/2 for i in x], opt_vals, width, label='Optimized', alpha=0.7)
        ...
        ax4.bar([i - width/2 for i in x], current_risks, width, label='Current', alpha=0.7)
        ax4.bar([i + width/2 for i in x], opt_risks, width, label='Optimized', alpha=0.7)
